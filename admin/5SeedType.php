                ...
                <div id="tab1" class="tab-content active">
                    <div class="card-body">
                        <div style="max-height: 430px; overflow: auto; width: 100%; border: 1px solid #ddd; border-radius: 10px; box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1);">
                            <table id="dataTable5" class="table" class="table table-bordered text-center" width="100%" cellspacing="0">
...
if ($resultStation->num_rows > 0) {
    ...
    if (!$stmt_count) {
        die("Error in COUNT SQL Query: " . $conn->error);
    }
    ...
    if ($result_count && $row_count = $result_count->fetch_assoc()) {
        $total_entries = (int) $row_count['total']; // Convert to integer to prevent issues
    } else {
        $total_entries = 0; // Ensure no errors if query fails
    }

    $stmt_count->close();

    // ** Fix: Check before using ceil() **
    $total_pages = ($total_entries > 0) ? ceil($total_entries / $entries_per_page) : 1;

    // Fetch paginated seed types
